...
from kabosu_core.language.ja.nlp.bunkai.algorithm.lbd.corpus import LABEL_NSEP, LABEL_OTHER, LABEL_SEP, annotation2spans
...
def count(path: Path, outf: typing.IO, show: bool = False) -> Statics:
    ...
    with path.open() as inf:
        for doc in inf:
            text = doc[:-1]
            st.sentence += text.count(kabosu_core.language.ja.nlp.bunkai.constant.METACHAR_SENTENCE_BOUNDARY)
