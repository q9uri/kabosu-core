...
def get_text(
    ...
    language_str_bak = language_str
    language_str, norm_text, phone, tone, word2ph, _, _, _ = clean_text_with_given_phone_tone(
        text,
        language_str,
    ...
    if language_str_bak == Languages.MULTI:
        bert = bert_ori
        ja_bert = bert_ori
        en_bert = bert_ori
    
    elif language_str_bak == Languages.ZH:
        bert = bert_ori
        ja_bert = torch.zeros(1024, len(phone))
        en_bert = torch.zeros(1024, len(phone))
    elif language_str_bak == Languages.JP:
        bert = torch.zeros(1024, len(phone))
        ja_bert = bert_ori
        en_bert = torch.zeros(1024, len(phone))
    elif language_str_bak == Languages.EN:
        bert = torch.zeros(1024, len(phone))
        ja_bert = torch.zeros(1024, len(phone))
        en_bert = bert_ori
    else:
