...
def initialize(
    ...
    freeze_style: bool,
    freeze_decoder: bool,
    support_language: Literal["base", "use_jp_extra", "base_v2"],
    log_interval: int,
):
    ...
    if support_language == "use_jp_extra":
        default_config_path = "configs/config_jp_extra.json"
        use_jp_extra = True

    elif support_language == "base_v2":
        default_config_path = "configs/config_v2.json"
        use_jp_extra = False
    else:
...
def bert_gen(model_name: str, use_multiringual: bool):
    logger.info("Step 4: start bert_gen...")
    config_path = get_path(model_name).config_path

    cmd = ["tools/bert_gen.py", "--config", str(config_path)]
    if use_multiringual:
        cmd.append("--use_multilingual")
...
def preprocess_all(
    ...
    if support_language == "base_v2":
        use_multiringual = True
    else:
        use_multiringual = False

    success, message = bert_gen(
        model_name=model_name,
        use_multiringual=use_multiringual
...
def train(
    ...
    support_language :Literal["base", "use_jp_extra", "base_v2"] = "base",
    speedup: bool = False,
    ...
    if support_language == "use_jp_extra":
        train_py = "tools/train_ms.py"
    
    elif support_language == "base_v2":
        train_py = "tools/train_ms_v2.py"

    else:
        train_py = "tools/train_ms_jp_extra.py"
