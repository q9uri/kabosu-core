...
def extract_bert_feature(
    ...
    if language in (Languages.JP, Languages.KO):
    ...
    elif language == Languages.MULTI:
        from kabosu_plus.sbv2.nlp.multiringual.bert_feature import extract_bert_feature_lammacpp

        out = extract_bert_feature_lammacpp(
            text,
            word2ph,
            assist_text,
            assist_text_weight,
...
def extract_bert_feature_onnx(
    ...
    if language in (Languages.JP, Languages.KO):
        ...
        return extract_bert_feature_lammacpp(
            text,
            word2ph,
            assist_text,
            assist_text_weight,
...
def _clean_text(
    ...
    if language in ("ZH", "JP", "EN", "MULTI", "KO"):
        if language == "KO":
            language = Languages.JP

        from kabosu_plus.sbv2.nlp.multiringual.g2p import g2p
        language, norm_text, phones, tones, word2ph, sep_text, sep_kata, sep_kata_with_joshi = g2p(text,
...
def clean_text_with_given_phone_tone(
    ...
    # 与えられたテキストをクリーニング
    language, norm_text, phone, tone, word2ph, sep_text, sep_kata, sep_kata_with_joshi = (
        _clean_text(
            text,
            language,
            use_jp_extra=use_jp_extra,
    ...
    return language, norm_text, phone, tone, word2ph, sep_text, sep_kata, sep_kata_with_joshi
...
def language_selector(text: str, support_languages: Literal[Languages]):
    from kabosu_plus.sbv2.nlp import language_selector
    return language_selector(text, support_languages)